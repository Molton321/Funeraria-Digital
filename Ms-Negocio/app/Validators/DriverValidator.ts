...
export default class DriverValidator {
  ...
  public schema = schema.create({
    ...
    user_id: schema.string([rules.unique({ table: 'administrators', column: 'user_id', where: {user_id: this.ctx.request.input('user_id')} }), rules.unique({ table: 'administrators', column: 'user_id' }), rules.unique({ table: 'clients', column: 'user_id', where: {user_id: this.ctx.request.input('user_id')} })]),
  })
